// Macro to create composite with wavelength choice.
...
firstSub = substring (repertoire, 0 ,slashIndex);print(firstSub);
		...
		for (i=0;i<nWavelength;i++)
			{
				if (stackOrNot[i] == 1)
					{
						stacking[i] = Dialog.getRadioButton();
					}
			}
...
for (index=firstImage; index<=lastImage; index++)
{
	...
	//image treatment
		...
		imageTreatment(repertoire, name, nColours, colours, stacking);
...
// Merge channels
			...
			if (color[i] == channel[0]){mergeColor = ""+mergeColor +"c5=["+pref+imageName[i]+"]"; } //Far Red
			else if (color[i] == channel[1]){ mergeColor = ""+mergeColor + "c1=["+pref+imageName[i]+"]";} //Red
			else if (color[i] == channel[2]){ mergeColor = ""+mergeColor + "c2=["+pref+imageName[i]+"]";} //Green
			else if (color[i] == channel[3]){ mergeColor = ""+mergeColor + "c3=["+pref+imageName[i]+"]";} //Blue
			else if (color[i] == channel[4]){ mergeColor = ""+mergeColor + "c4=["+pref+imageName[i]+"]";} //Phase
			else if (color[i] == channel[5]){ mergeColor = ""+mergeColor + "c4=["+pref+imageName[i]+"]";} //DIC
		}
