...
if(!empty($_POST)){
  ...
  if(isset($_POST['protoc'])){
    ...
    if($validator->isValid()){
      $db = new Database;
      $validator->isUniq('name',$db,App::getTableSpacvirProtocols(),"The protocol already exists.");
    }
    $validator->isSelect('category', "Please choose a category for your protocol.");

    if($validator->isValid()){

      $protocol = new Protocol("spacvir");
  ...
  elseif(isset($_POST['catego'])){
    ...
    if($validator->isValid()){
      $db = new Database;
      $validator->isUniq('category',$db,App::getTableSpacvirProtocolCategories(),"The category already exists.");
    }
    if($validator->isValid()){
      $protocol = new Protocol("spacvir");
